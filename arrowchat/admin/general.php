	...
	*/
	...
	if (var_check('chatsettings_submit')) 
	{
		$result = $db->execute("
			...
			END WHERE config_name IN ('disable_avatars', 'disable_smilies', 'disable_arrowchat', 'disable_buddy_list', 'search_number', 'chat_maintenance', 'admin_chat_all', 'admin_view_maintenance', 'guests_can_view', 'guests_can_chat', 'guests_chat_with', 'users_chat_with', 'show_full_username', 'us_time')
		");
	...
	if (var_check('chatstyle_submit')) 
	{
		...
		if (empty($error)) 
		{
			$result = $db->execute("
				...
				END WHERE config_name IN ('width_buddy_list', 'width_chatrooms', 'width_applications', 'bar_fixed', 'bar_fixed_width', 'bar_fixed_alignment', 'bar_padding')
			");
