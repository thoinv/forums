...
$__extraData['title'] = '';
$__extraData['title'] .= htmlspecialchars($page['page_name'], ENT_QUOTES, 'UTF-8') . ' - ' . 'Edit';
$__output .= '
';
$__extraData['h1'] = '';
$__extraData['h1'] .= htmlspecialchars($page['page_name'], ENT_QUOTES, 'UTF-8') . ' - ' . 'Edit';
$__output .= '
...
<div class="sectionMain wikiEdit">
	<form action="' . XenForo_Template_Helper_Core::link('wiki/edit', $page, array()) . '" method="post" class="xenForm">
		<fieldset>
			<dl class="ctrlUnit">
				<dt><label for="ctrl_name">' . 'Title' . ':</label></dt>
				<dd><input type="text" name="page_name" class="textCtrl SlugIn" id="ctrl_name" maxlength="50" value="' . htmlspecialchars($page['page_name'], ENT_QUOTES, 'UTF-8') . '" /></dd>
			</dl>
			...
			<dl class="ctrlUnit">
				...
				<dd>
					';
if ($page['page_slug'] == ('index'))
{
$__output .= '
						<input type="text" name="page_slugNew" class="textCtrl" id="ctrl_slug" maxlength="50" value="' . htmlspecialchars($page['page_slug'], ENT_QUOTES, 'UTF-8') . '" READONLY />
					';
}
else
{
$__output .= '
						<input type="text" name="page_slugNew" class="textCtrl SlugEdit SlugOut" id="ctrl_slug" maxlength="50" value="' . (($input['page_slug']) ? (htmlspecialchars($input['page_slug'], ENT_QUOTES, 'UTF-8')) : (htmlspecialchars($page['page_slug'], ENT_QUOTES, 'UTF-8'))) . '" />
					';
}
$__output .= '
				</dd>
...
$__output .= '
				...
				<dl class="ctrlUnit">
					...
					<dd><input type="text" name="page_content" class="textCtrl" id="ctrl_content" maxlength="50" value="' . htmlspecialchars($page['page_content'], ENT_QUOTES, 'UTF-8') . '" /></dd>
				</dl>
...
$__output .= '
				<dl class="ctrlUnit">
					...
					<dd><select name="page_type" id="ctrl_type" class="textCtrl autoSize">
						<option value="bbcode" ' . (($page['page_type'] == ('bbcode')) ? ' selected="selected"' : '') . '>' . 'BB Codes' . '</option>
						';
if ($perms['admin'])
{
$__output .= '<option value="html" ' . (($page['page_type'] == ('html')) ? ' selected="selected"' : '') . '>' . 'HTML' . '</option>';
}
$__output .= '
					</select></dd>
				</dl>
...
$__output .= '
			...
			<dl class="ctrlUnit">
				<dt><label for="page_parent">' . 'Parent Node' . ':</label></dt>
...
$__output .= '
						<option value="' . htmlspecialchars($list['page_id'], ENT_QUOTES, 'UTF-8') . '"
							' . (($page['page_parent'] == $list['page_id']) ? ' selected="selected"' : '') . '
...
$__output .= '
				...
				';
if ($perms['delete'] && $page['page_slug'] != ('index'))
{
$__output .= '
					<a href="' . XenForo_Template_Helper_Core::link('wiki/delete', $page, array()) . '" type="button" class="button OverlayTrigger">' . 'Delete Page' . '...</a>
				';
}
$__output .= '
			</dd>
...
$__output .= '
			<fieldset>
				<dl class="ctrlUnit">
					...
					<dd><ul>
						<li>
							<label for="ctrl_index"><input type="text" name="page_index" class="textCtrl" id="ctrl_index" maxlength="2" value="' . htmlspecialchars($page['page_index'], ENT_QUOTES, 'UTF-8') . '" style="width: 18px;" /> ' . 'Wiki Index Rating' . '</label>
							<p class="hint">' . 'Use "0" to leave a page off the index.' . '</p>
						</li>

						<li>
							<label for="ctrl_sidebar"><input type="checkbox" name="page_sidebar" value="1" id="ctrl_sidebar" ' . (($page['page_sidebar']) ? ' checked="checked"' : '') . ' /> ' . 'Show Page Sidebar Block' . '</label>
						</li>
						<li>
							<label for="ctrl_sublist"><input type="checkbox" name="page_sublist" value="1" id="ctrl_sublist" ' . (($page['page_sublist']) ? ' checked="checked"' : '') . ' /> ' . 'Show Page Children List' . '</label>
						</li>
						<li>
							<label for="ctrl_protect"><input type="checkbox" name="page_protect" value="1" id="ctrl_protect" ' . (($page['page_protect']) ? ' checked="checked"' : '') . ' /> ' . 'Mark Page as Protected' . '</label>
...
$__output .= '
			...
			<dl class="ctrlUnit">
				...
				<dd><input type="search" name="administrators" placeholder="' . 'Name' . '..." value="' . htmlspecialchars($page['page_admins'], ENT_QUOTES, 'UTF-8') . '" results="0" class="textCtrl AutoComplete" />
			...
			<fieldset>
				<dl class="ctrlUnit">
					...
					<dd><input type="search" name="usernames" placeholder="' . 'Name' . '..." value="' . htmlspecialchars($page['page_users'], ENT_QUOTES, 'UTF-8') . '" results="0" class="textCtrl AutoComplete" /></dd>
				</dl>
...
$__output .= '
		...
		<input type="hidden" name="timestamp" value="' . htmlspecialchars($page['timestamp'], ENT_QUOTES, 'UTF-8') . '" />
		<input type="hidden" name="_xfToken" value="' . htmlspecialchars($visitor['csrf_token_page'], ENT_QUOTES, 'UTF-8') . '" />
	</form>
</div>

';
if ($perms['admin'] && $forums)
{
$__output .= '
<div class="sectionMain">
	<form action="' . XenForo_Template_Helper_Core::link('wiki/create-thread', $page, array()) . '" method="post" class="xenForm">
		<dl class="ctrlUnit">
			<dt><label for="ctrl_name">' . 'Discussion Thread' . ':</label></dt>
			<dd>
				';
if ($thread)
{
$__output .= '
					<a href="' . XenForo_Template_Helper_Core::link('threads', $thread, array()) . '">' . htmlspecialchars($thread['title'], ENT_QUOTES, 'UTF-8') . '</a>
				';
}
else
{
$__output .= '
					<select name="wiki_node" class="textCtrl autoSize">
					';
foreach ($forums AS $forum)
{
$__output .= '
						<option value="' . htmlspecialchars($forum['node_id'], ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($forum['title'], ENT_QUOTES, 'UTF-8') . '</option>
					';
}
$__output .= '
					</select>
					 &nbsp; &nbsp; 
					<input type="submit" value="' . 'Create Thread' . '" name="submit" accesskey="s" class="button primary" />
				';
}
$__output .= '
			</dd>
