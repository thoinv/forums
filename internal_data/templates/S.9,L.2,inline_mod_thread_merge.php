...
<form action="' . XenForo_Template_Helper_Core::link('inline-mod/thread/merge', false, array()) . '" method="post" class="xenForm formOverlay">
	<p>' . 'Bạn có chắc muốn gộp ' . htmlspecialchars($threadCount, ENT_QUOTES, 'UTF-8') . ' chủ đề?' . '</p>
...
$__output .= '
	...
	<dl class="ctrlUnit">
		...
		<dd><ul>
			<li>
				<label><input type="checkbox" name="send_starter_alert" value="1" checked="checked" class="Disabler" id="ctrl_send_starter_alert" /> ' . 'Notify non-destination thread starters of this action.' . ' ' . 'Lý do' . ':</label>
