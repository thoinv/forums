...
<form action="' . XenForo_Template_Helper_Core::link('threads/poll/edit', $thread, array()) . '" method="post" class="xenForm">
	<dl class="ctrlUnit">
		<dt><label for="ctrl_question">' . 'Câu hỏi' . ':</label></dt>
		<dd>
			';
...
{
$__output .= '
						<input type="text" name="existing_responses[' . htmlspecialchars($pollResponseId, ENT_QUOTES, 'UTF-8') . ']" value="' . htmlspecialchars($response['response'], ENT_QUOTES, 'UTF-8') . '" class="textCtrl" placeholder="' . 'Lựa chọn' . '..." maxlength="100" />
					';
}
else
{
...
{
$__output .= '
				<li><input type="text" name="new_responses[]" class="textCtrl" placeholder="' . 'Lựa chọn' . '..." maxlength="100" /></li>
			';
}
$__output .= '
			</ul>
			<input type="button" value="' . 'Thêm lựa chọn trả lời' . '" class="FieldAdder button smallButton" data-source="ul.PollResponseContainer li" data-maxfields="' . htmlspecialchars($xenOptions['pollMaximumResponses'], ENT_QUOTES, 'UTF-8') . '" />
		</dd>
	</dl>
...
{
$__output .= '
					<li><label><input type="checkbox" name="public_votes" value="1" ' . (($poll['public_votes']) ? ' checked="checked"' : '') . ' /> ' . 'Hiển thị bình chọn công cộng' . '</label></li>
				';
}
$__output .= '
...
{
$__output .= '
					<li><label><input type="checkbox" name="close" value="1" class="Disabler" id="ctrl_close" /> ' . 'Đóng bình chọn này sau' . ':</label>
...
$__output .= '
	<form action="' . XenForo_Template_Helper_Core::link('threads/poll/reset', $thread, array()) . '" method="post" class="xenForm">
		...
		<dl class="ctrlUnit">
			...
			<dd>
				<ul>
					<li><label><input type="radio" name="poll_action" value="" checked="checked" /> ' . 'Không làm gì' . '</label><li>
					';
if ($poll['voter_count'])
{
$__output .= '
...
$__output .= '
		...
		<dl class="ctrlUnit submitUnit">
			<dt></dt>
			<dd>
				<input type="submit" value="' . 'Xóa' . '" class="button primary" />
			</dd>
		</dl>
