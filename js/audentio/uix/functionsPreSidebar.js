...
uix.sticky = {
	...
	add: function(itemName, normalHeight, stickyHeight, options){
		...
		item.normalHeight = normalHeight;
		item.stickyHeight = stickyHeight;
		...
		uix.sticky.stickyOffsetDist += normalHeight;
	...
	stick: function(index, currentHeight){
		...
		if (uix.sticky.items[index].scrollSticky){
			...
			innerWrapper.css({'top': currentHeight, 'z-index': 250} );
		}
	...
	delayUnstick: function(){ // unsticks everything with state == 2, used for scroll sticky
		for (var x=0; x<uix.sticky.items.length; x++) {
			if (uix.sticky.items[x].state == 2) uix.sticky.unstick(x);
		}
		$('.activeSticky').last().addClass('lastSticky');
		...
		for (var x=0; x<uix.sticky.items.length; x++) {
			if (uix.sticky.items[x].enabled){
				...
				if (uix.sticky.scrollDetectorRunning && uix.sticky.items[x].scrollSticky == 1) {
					if (uix.sticky.scrollTop > uix.sticky.stickyMinPos && uix.sticky.scrollDirection == "down" && (itemFromWindowTop < wrapperFromWindowTop && uix.sticky.scrollDistance > uix.sticky.stickyMinDist)) innerWrapper.css({
		...
		for (var x = 0; x < uix.sticky.items.length; x++){
			if (uix.sticky.items[x].state == 1 && uix.sticky.items[x].enabled){
				...
				if (uix.sticky.items[x].scrollSticky){
					...
					innerWrapper.css({'top': currentTop, 'z-index': 250} );
				}
...
$(document).ready(function(){
	...
	uix.sidebarSticky = {
		running:false,
		update: function(){
			if (uix.elm.sidebar.length && uix.stickySidebar && uix.windowWidth > uix.maxResponsiveWideWidth){
		...
		check: function(){
			if (uix.elm.sidebar.length && uix.stickySidebar && uix.windowWidth > uix.maxResponsiveWideWidth){
				...
				if (uix.sidebarSticky.mainContainerHeight>=uix.sidebarSticky.sidebarHeight) {
					var
						sidebarFromWindowTop = uix.sidebarSticky.sidebarOffset.top - (uix.sticky.stickyHeight+uix.scrollTop),
		...
		unstick: function(){
			if (uix.elm.sidebar.length && uix.stickySidebar && uix.windowWidth > uix.maxResponsiveWideWidth){
				...
				uix.stickySidebarState = 0;
				uix.stickySidebarBottomFixed = 0;
				if (uix.slidingSidebar){
					uix.sidebarSticky.innerWrapper.css({
						transition: 'top 0.2s', top: '', left: ''
					});
				} else {
					uix.sidebarSticky.innerWrapper.css({
						transition: 'top 0.0s', top: '', left: ''
					});
				}
		...
		fixBottom: function(){
			if (uix.elm.sidebar.length && uix.stickySidebar && uix.windowWidth > uix.maxResponsiveWideWidth){
				...
				uix.elm.sidebar.removeClass('sticky');
				uix.sidebarSticky.innerWrapper.css({
					transition: 'top 0s', top: uix.sidebarSticky.maxTop, left: ''
				});
	...
	uix.fn = {
		checkRadius: function(){
			...
			var _elms = ["#logoBlock .pageContent", "#content .pageContent", "#userBar .pageContent", "#navigation .pageContent", ".footer .pageContent", "#uix_footer_columns .pageContent", ".footerLegal .pageContent"];
			var elms = _elms;
			var elmInfo = {};
			...
			for (var i = 0; i < _elms.length; i++) { // Loop through all
				if(elmInfo[i]['length']){
					if (elmInfo[i]['width'] == windowWidth){ //Reset border-radius if element is full width
						...
						for (var x = 0; x < (elms.length - i); x++) { // Check if our element is touching others in elms[]
							if (elmInfo[x]['length']){
								if(x != i) { // Dont check against itself
									...
									if (isAttachedToTop) elmInfo[x]['bottomRadius'] = false;
									if (isAttachedToBottom) elmInfo[x]['topRadius'] = false;
								}
							}
						}
					}
		...
		jumpToFixed: function(){
			if (uix.performanceLog) performanceTimer = Date.now();
			if (uix.jumpToFixed_delayHide && uix.jumpToScrollTimer) clearTimeout(uix.jumpToScrollTimer);   // clear any previous pending timer

			if (uix.scrollTop > 140) {
				if (uix.jumpToFixedOpacity == 0) {
					uix.jumpToFixedOpacity = 1;
					uix.elm.jumpToFixed.css({'opacity' : '1'});
				}
		...
		fixNotices: function() {
			...
			if (api) api.seekTo(api.getIndex(), 0);
		}
	}
		...
		if ( $('.moderatorTabs').length ){
			...
			if ( $('.moderatorTabs .admin').length ) {
				...
				if (adminListTotal > 0) {
					...
					if (adminListTotalUnread) $('.moderatorTabs .admin .itemCount').addClass('alert');
				}
			}
			...
			sidebarCollapse.find('a').on('click', function(e) {
				...
				if ( mainSidebar.is(":visible") ) {
					...
					if (uix.windowWidth > documentWidthWhenSidebarResponsive) {
						mainSidebar.css({'opacity'		: '0'});
				   		window.setTimeout(function(){
				 			...
				 			window.setTimeout(function(){uix.fn.fixNotices();}, 400); // fix notices
				  		}, 400);
					} else {
						mainSidebar.hide();
						mainContent.css(marginDirection, 0);
					...
					sidebarCollapsePhrase.text(uix.collapsibleSidebar_phrase_close);
				  	var stickyCondition = (uix.stickySidebar && uix.windowWidth > uix.maxResponsiveWideWidth && !uix.sidebarSticky.running);
	...
	uix.init.sticky = function(){
		...
		for (var i = 0; i < stickySel.length; i++) {
			item = "#" + stickySel[i].id
			if (uix.stickyItems[item] !== undefined) sticky.add(item, uix.stickyItems[item].normalHeight, uix.stickyItems[item].stickyHeight, uix.stickyItems[item].options);
		}
	...
	uix.on('init', function(){
		...
		if (uix.elm.sidebar.length && uix.stickySidebar && uix.windowWidth > uix.maxResponsiveWideWidth && uix.elm.sidebar.is(":visible")) {
			uix.init.stickySidebar();
		}
