...
class HQCoder_ThreadRating_ControllerPublic_Thread extends XFCP_HQCoder_ThreadRating_ControllerPublic_Thread
{
	public function actionIndex()
	{
		return $this->_getHQCoder_ThreadRatingPermission(parent::actionIndex());
	}
	...
	public function actionRate()
	{
		...
		*/
		...
		$ratingDw = XenForo_DataWriter::create('HQCoder_ThreadRating_DataWriter_Rating', XenForo_DataWriter::ERROR_EXCEPTION);
		...
		if ($existing)
		{
			$deleteDw = XenForo_DataWriter::create('HQCoder_ThreadRating_DataWriter_Rating');
	...
	public function actionWhoRated()
	{
		...
		if (!$whoRated)
		{
			return $this->responseError(new XenForo_Phrase('HQCoder_ThreadRating_no_one_has_rated_this_thread_yet'));
		}
		...
		return $this->responseView('HQCoder_ThreadRating_ViewPublic_WhoRated', 'HQCoder_ThreadRating_whorated', $viewParams);
	}
	...
	public function actionLive()
	{
		...
		{
			$response = $this->_getHQCoder_ThreadRatingPermission(parent::actionLive());
		} else {
	...
	protected function _getThreadForumFetchOptions()
	{
		...
		$threadFetchOptions['join'] += HQCoder_ThreadRating_Model_Thread::FETCH_THREADRATE;
	...
	private function _getHQCoder_ThreadRatingPermission($response)
	{
		if ($response instanceof XenForo_ControllerResponse_View AND is_array($response->params))
		{
			$visitor = XenForo_Visitor::getInstance();

			$response->params['HQCoder_ThreadRating']['canRate'] = (
			...
			$response->params['HQCoder_ThreadRating']['canView'] = $visitor->hasPermission('general', 'tr_viewRatings');
			$response->params['HQCoder_ThreadRating']['whoRated'] = $visitor->hasPermission('general', 'tr_whoRated');
		}
