...
class Nobita_Teams_DataWriter_TeamField extends XenForo_DataWriter
{
	const ADDON_ID = '';
	...
	protected function _getFields()
	{
		return array(
			'xf_team_field' => array(
				...
				/*'display_group'         => array('type' => self::TYPE_STRING, 'default' => 'extra_tab',
					'allowedValues' => array('extra_tab', 'new_tab')
				),*/
				'display_group'         => array('type' => self::TYPE_STRING, 'default' => 'extra_tab',
					'allowedValues' => array('extra_tab', 'new_tab', 'parent_tab')
				),
				...
				'match_type'            => array('type' => self::TYPE_STRING, 'default' => 'none',
					'allowedValues' => array('none', 'number', 'alphanumeric', 'email', 'url', 'regex')
				),
				...
				// 1.0.7 @truonglv
				'parent_tab_id' => array('type' => self::TYPE_STRING, 'default' => '', 'verification' => array('$this', '_verifyParentTabId'))
			)
		);
	}
	...
	*/
	protected function _getExistingData($data)
	{
		...
		return array('xf_team_field' => $this->_getFieldModel()->getTeamFieldById($id));
	}
	...
	*/
	...
	protected function _verifyFieldId(&$id)
	{
		...
		if ($id !== $this->getExisting('field_id') && $this->_getFieldModel()->getTeamFieldById($id))
		{
	...
	protected function _verifyParentTabId(&$tabId)
	{
		if (!empty($tabId))
		{
			$existing = $this->_getFieldModel()->getTeamFieldById($tabId);
			if (empty($existing))
			{
				$this->error(new XenForo_Phrase('Teams_requested_field_not_found'), 404);
				return false;
			}
			
			if ($existing['display_group'] != 'new_tab')
			{
				$this->error(new XenForo_Phrase('Teams_this_field_can_not_have_child_tab'));
				return false;
			}
		}
	...
	protected function _preSave()
	{
		...
		{
			$typeMap = $this->_getFieldModel()->getTeamFieldTypeMap();
	...
	protected function _updateCategoryAssociations(array $categoryIds)
	{
		...
		$db->delete('xf_team_field_category', 'field_id = ' . $db->quote($fieldId));
	...
	protected function _postDelete()
	{
		...
		$this->_db->delete('xf_team_field_value', 'field_id = ' . $this->_db->quote($fieldId));
		// note the team caches aren't rebuilt here; this shouldn't be an issue as we don't enumerate them

		$this->_db->delete('xf_team_field_category', 'field_id = ' . $this->_db->quote($fieldId));
	...
	{
		return $this->_getFieldModel()->getTeamFieldTitlePhraseName($id);
	}
	...
	{
		return $this->_getFieldModel()->getTeamFieldDescriptionPhraseName($id);
	}
	...
	{
		return $this->_getFieldModel()->getTeamFieldChoicePhraseName($fieldId, $choice);
	}

	protected function _rebuildTeamFieldCache()
	{
		return $this->_getFieldModel()->rebuildTeamFieldCache();
	}

	/**
	 * @return Nobita_Teams_Model_Field
	 */
