...
class XenForo_ControllerAdmin_Cron extends XenForo_ControllerAdmin_Abstract
{
	...
	public function actionRun()
	{
		...
		if ($entry['addon_id'] && $entry['addon_id'] != 'XenForo')
		{
			$addOn = $this->_getAddOnModel()->getAddOnById($entry['addon_id']);
			if (!$addOn || !$addOn['active'])
			{
				return $this->responseError(new XenForo_Phrase('cron_entry_associated_with_inactive_add_on_cannot_run'));
			}
		}
	...
	/**
	 * Selectively enables or disables specified cron task
	 *
	 * @return XenForo_ControllerResponse_Abstract
	 */
	public function actionToggle()
	{
		return $this->_getToggleResponse(
			$this->_getCronModel()->getAllCronEntries(),
