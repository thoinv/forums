...
class XenForo_DataWriter_Poll extends XenForo_DataWriter
{
	...
	protected function _getFields()
	{
		return array(
			'xf_poll' => array(
				...
				'max_votes'    => array('type' => self::TYPE_UINT_FORCED,    'default' => 0, 'max' => 255),
				'close_date'   => array('type' => self::TYPE_UINT,    'default' => 0),
				'change_vote'  => array('type' => self::TYPE_BOOLEAN, 'default' => 0),
				'view_results_unvoted' => array('type' => self::TYPE_BOOLEAN, 'default' => 1),
			)
		);
	}
	...
	*/
	...
	protected function _preSave()
	{
		...
		if ($this->isUpdate()
			&& $this->get('voter_count')
			&& $this->isChanged('max_votes')
			&& $this->get('max_votes') != 0 // unlimited is always ok
		)
		{
			$newMax = $this->get('max_votes');
			$oldMax = $this->getExisting('max_votes');
			if ($oldMax == 0 || $newMax < $oldMax)
			{
				$this->error(new XenForo_Phrase('maximum_selectable_responses_value_may_not_be_reduced'), 'max_votes');
			}
		}
	}
