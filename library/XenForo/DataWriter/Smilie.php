...
class XenForo_DataWriter_Smilie extends XenForo_DataWriter
{
		...
		return array(
			'xf_smilie' => array(
				...
				'smilie_text' => array('type' => self::TYPE_STRING, 'required' => true,
					'verification' => array('$this', '_verifySmilieText'),
					'requiredError' => 'please_enter_valid_smilie_text'
				),
				...
				// XF 1.3 smilie enhancements
				'smilie_category_id' => array('type' => self::TYPE_UINT,
					'verification'   => array('$this', '_verifySmilieCategory')
				),
				'display_order'      => array('type' => self::TYPE_UINT),
				'display_in_editor'  => array('type' => self::TYPE_BOOLEAN, 'default' => true)
			)
		);
	}
	...
	*/
	...
	/**
	 * Verifies that smilie category is valid
	 *
	 * @param integer $smilieCategoryId
	 *
	 * @return boolean
	 */
	protected function _verifySmilieCategory(&$smilieCategoryId)
	{
		if (!empty($smilieCategoryId))
		{
			// verify that the category exists
			$smilieCategory = $this->_getSmilieModel()->getSmilieCategoryById($smilieCategoryId);

			if (empty($smilieCategory))
			{
				$this->error(new XenForo_Phrase('please_enter_valid_smilie_category_id'), 'smilie_category_id');
				return false;
			}
		}
