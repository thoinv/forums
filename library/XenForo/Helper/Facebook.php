...
class XenForo_Helper_Facebook
{
	...
	public static function getAccessToken($url, $code)
	{
		...
		try
		{
			$client = XenForo_Helper_Http::getClient('https://graph.facebook.com/v2.0/oauth/access_token');
	...
	public static function getUserInfo($accessToken, $path = 'me')
	{
		try
		{
			$client = XenForo_Helper_Http::getClient('https://graph.facebook.com/v2.0/' . $path);
			if ($accessToken)
			{
	...
	public static function getUserPicture($accessToken, $size = 'large')
	{
		try
		{
			$client = XenForo_Helper_Http::getClient('https://graph.facebook.com/v2.0/me/picture?type=' . $size);
			$client->setParameterGet('access_token', $accessToken);
