...
class XenForo_Model_Like extends XenForo_Model
{
	...
	public function getLikesByUserSinceDate($userId, $cutOff, $limit = null)
	{
		return $this->fetchAllKeyed($this->limitQueryResults(
			'
				SELECT liked_content.*
				...
				WHERE liked_content.like_user_id = ?
					AND liked_content.like_date > ?
				ORDER BY liked_content.like_date
			', $limit
		), 'like_id', array($userId, $cutOff));
	}

	public function countLikesByUserSinceDate($userId, $cutOff)
	{
		return $this->_getDb()->fetchOne('
			SELECT COUNT(*)
			FROM xf_liked_content AS liked_content
