...
abstract class XenForo_Search_DataHandler_Abstract
{
	...
	public function getSearchContentTypePhrase()
	{
		...
		return new XenForo_Phrase(reset($types));
	}

	/**
	 * If inline mod is supported in search results, return these keys
	 *  * name - name of content type to display
	 *  * route - route to "switch"
	 *  * cookie - name of inline mod cookie used (not including prefixes or "inlinemod_")
	 *  * template - template that contains all the options
	 *
	 * @return null|array
	 */
	...
	protected $_inlineModEnabled = false;

	/**
	 * @param bool $value
	 */
	public function setInlineModEnabled($value)
	{
		$this->_inlineModEnabled = (bool)$value;
	}
	...
	/**
	 * If relevant, add the inline moderation related option to the search
	 * result record and return the options that are applicable.
	 *
	 * @param array $result
	 *
	 * @return array
	...
	public final function deleteFromIndex(XenForo_Search_Indexer $indexer, array $dataList)
	{
		...
		list($k, $first) = each($dataList);

		if (!is_array($first) && is_string($k))
		{
