...
class XenResource_ControllerPublic_Resource extends XenForo_ControllerPublic_Abstract
{
	...
	public function actionRate()
	{
		...
		if ($this->isConfirmedPost())
		{
			$hasReview = strlen($input['message']) > 0;
			if (XenForo_Application::getOptions()->resourceReviewRequired && !$hasReview)
			{
				return $this->responseError(new XenForo_Phrase('please_provide_review_with_your_rating'));
			}

			$minLength = XenForo_Application::getOptions()->resourceMinimumReviewLength;
			if ($hasReview && utf8_strlen($input['message']) < $minLength)
			{
				return $this->responseError(new XenForo_Phrase('your_review_must_be_at_least_x_characters', array('min' => $minLength)));
			}
	...
	public function actionSaveUpdate()
	{
		...
		if ($this->_noRedirect() && !$this->_input->filterSingle('force_redirect', XenForo_Input::BOOLEAN) && $updateId)
		{
		...
		else
		{
			return $this->responseRedirect(
				XenForo_ControllerResponse_Redirect::SUCCESS,
				XenForo_Link::buildPublicLink('resources/update', $resource, array('update' => $dw->get('resource_update_id')))
			);
		}
	}
