 ...
 */
class sonnb_XenGallery_Importer_PhotopostProVb extends sonnb_XenGallery_Importer_Abstract
{
	public static function getName()
	{
		return 'PhotoPost Pro 8.4 (vBB Integration) => sonnb - XenGallery';
	}
		...
		if ($config)
		{
			...
			if (empty($config['data_path']))
			{
				...
				return $controller->responseView(
					'XenForo_ViewAdmin_Import_Pppvb_Config',
					'sonnb_xengallery_import_pppvb_config',
					array(
						'config' => $config,
	...
	public function stepAlbums($start, array $options)
	{
		...
		foreach ($albums AS $album)
		{
			...
			if ($album['cattype'] == 'a' && !isset($userIdMap[$album['userid']]))
			{
				continue;
			}
			...
			if ($last <> $next)
			{
				...
				switch ($album['cattype'])
				{
					...
					case 'a':
						...
						$username = $this->_convertToUtf8($album['user'], true);
