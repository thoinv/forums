 ...
 */
class sonnb_XenGallery_Model_AmazonECS
{
	...
	private $responseConfig = array (
		'returnType' => self::RETURN_TYPE_OBJECT, 'responseGroup' => 'Small', 'optionalParameters' => array ()
	);
	/**
	...
	public function search($pattern, $nodeId = null)
	{
		...
		$params = $this->buildRequestParams('ItemSearch', array_merge(array (
				'Keywords' => $pattern, 'SearchIndex' => $this->requestConfig['category']
			), $browseNode));
	...
	protected function buildRequestParams($function, array $params)
	{
		...
		return array_merge($associateTag, array (
				'AWSAccessKeyId' => $this->requestConfig['accessKey'], 'Request' => array_merge(array ('Operation' => $function), $params, $this->responseConfig['optionalParameters'], array ('ResponseGroup' => $this->prepareResponseGroup()))
			));
	}
	...
	protected function returnData($object)
	{
		switch ($this->responseConfig['returnType'])
		{
			...
			default:
				throw new InvalidArgumentException(sprintf("Unknwon return type %s", $this->responseConfig['returnType']));
				break;
		}
	}
	...
	protected function performSoapRequest($function, $params)
	{
		...
		if (ini_get('maximum_execution_time') < 60)
		{
			set_time_limit(60);
		}
	...
	protected function buildSoapHeader($function)
	{
		...
		return array (
			new SoapHeader('http://security.amazonaws.com/doc/2007-01-01/', 'AWSAccessKeyId', $this->requestConfig['accessKey']), new SoapHeader('http://security.amazonaws.com/doc/2007-01-01/', 'Timestamp', $timeStamp), new SoapHeader('http://security.amazonaws.com/doc/2007-01-01/', 'Signature', $signature)
		);
	}
	...
	public function lookup($asin)
	{
		...
		return $this->returnData($this->performSoapRequest("ItemLookup", $params));
	}
	...
	public function browseNodeLookup($nodeId)
	{
		...
		return $this->returnData($this->performSoapRequest("BrowseNodeLookup", $params));
	}
	...
	public function similarityLookup($asin)
	{
		...
		return $this->returnData($this->performSoapRequest("SimilarityLookup", $params));
	}
	...
	public function optionalParameters($params = null)
	{
		...
		{
			throw new InvalidArgumentException(sprintf("%s is no valid parameter: Use an array with Key => Value Pairs", $params));
		}
	...
	public function page($page)
	{
		...
		{
			throw new InvalidArgumentException(sprintf('%s is an invalid page value. It has to be numeric and positive', $page));
		}

		$this->responseConfig['optionalParameters'] = array_merge($this->responseConfig['optionalParameters'], array ("ItemPage" => $page));
