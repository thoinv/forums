 ...
 */
class sonnb_XenGallery_ModeratorLogHandler_Video extends XenForo_ModeratorLogHandler_Abstract
{
	/**
	 * @param array $logUser
	 * @param array $content
	...
	protected function _log(array $logUser, array $content, $action, array $actionParams = array(), $parentContent = null)
	{
		...
		$dw->bulkSet(array(
			...
			'content_type' => sonnb_XenGallery_Model_Video::$xfContentType,
			...
			'content_url' => XenForo_Link::buildPublicLink('gallery/videos', $content),
			'discussion_content_type' => sonnb_XenGallery_Model_Video::$xfContentType,
	...
	protected function _prepareEntry(array $entry)
	{
		...
		$entry['content_title'] = 'Video: '.$entry['content_title'];

		$entry['actionText'] = new XenForo_Phrase(
			'sonnb_xengallery_moderator_log_video_'.$entry['action'],
